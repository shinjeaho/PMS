...
def business(year: int):
    ...
    requested_view = (request.args.get('view', default='', type=str) or '').strip().lower()
    requested_status = (request.args.get('status', default='', type=str) or '').strip().lower()
    requested_tab = (request.args.get('tab', default='', type=str) or '').strip().lower()
